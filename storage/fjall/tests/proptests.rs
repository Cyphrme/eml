...
enum StorageAction {
    ...
    WriteBatch {
        ...
        nodes: Vec<(u64, u64, usize, Vec<u8>)>,
    },
}
...
fn arb_action() -> impl Strategy<Value = StorageAction> {
    prop_oneof![
        ...
        (any::<u64>(), any::<u64>(), 0..8usize, prop::collection::vec(any::<u8>(), 0..32))
            .prop_map(|(alg_id, left, height, hash)| StorageAction::StoreNode { alg_id, left, height, hash }),
        (any::<u64>(), prop::collection::vec((any::<u64>(), any::<u64>()), 0..4))
            .prop_map(|(alg_id, epochs)| StorageAction::StoreAlgorithmMeta { alg_id, epochs }),
        (
            prop::collection::vec((any::<u64>(), prop::collection::vec(any::<u8>(), 0..64)), 0..5),
            prop::collection::vec((any::<u64>(), any::<u64>(), 0..8usize, prop::collection::vec(any::<u8>(), 0..32)), 0..5)
...
impl StorageOracle {
    ...
    fn apply(&mut self, action: &StorageAction) {
        match action {
            ...
            StorageAction::StoreNode { alg_id, left, height, hash } => {
    ...
    fn expected_len(&self) -> u64 {
        self.leaves.keys().next_back().map(|&index| index + 1).unwrap_or(0)
    }
}
...
async fn run_differential_test(actions: Vec<StorageAction>) {
    ...
    for action in &actions {
        ...
        match action {
            ...
            StorageAction::StoreNode { alg_id, left, height, hash } => {
                storage.store_node(*alg_id, *left, *height, hash).await.unwrap();
            }
            StorageAction::StoreAlgorithmMeta { alg_id, epochs } => {
            ...
            StorageAction::WriteBatch { leaves, nodes } => {
                let leaves_ref: Vec<(u64, &[u8])> = leaves
                    ...
                    .collect();
                let nodes_ref: Vec<(u64, u64, usize, &[u8])> = nodes
                    .iter()
    ...
    for (&(alg_id, left, height), expected_hash) in &oracle.nodes {
        let actual_hash = storage.get_node(alg_id, left, height).await.unwrap().unwrap();
        assert_eq!(&actual_hash, expected_hash);
    }

    for &(alg_id, left, height) in oracle.nodes.keys() {
        if !oracle.nodes.contains_key(&(alg_id, left + 1, height)) {
            assert!(storage.get_node(alg_id, left + 1, height).await.unwrap().is_none());
        }
    }
...
proptest! {
    ...
    fn test_fjall_differential(ref actions in prop::collection::vec(arb_action(), 1..30)) {
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        rt.block_on(run_differential_test(actions.clone()));
    }
}
