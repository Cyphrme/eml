...
- **`resume_alg`$(a)$**: Reopens a frozen algorithm by appending a new
  ...
  subtrees. Cost: $\mathcal{O}(\log n)$. Resume is structurally
  necessary for deployments where algorithm activation is coupled to
  key lifecycle (e.g., a key using a previously removed algorithm is
  reactivated) rather than governed by a global deprecation policy.
