...
This is not one tree per algorithm. It is a single, unified data structure with multiple projections. While storing distinct internal node hashes for each algorithm is storage-equivalent to parallel logs, this shared-topology model yields three crucial advantages over running disjoint logs:
...
3. **Consolidated Auditing Complexity:** Auditors and monitors watch a single log identity. Equivocation on any projection is immediately detectable as a split-view of the same shared logical sequence, bypassing the distributed systems nightmare of reconciling separate, asynchronous log timelines.
...
<figure id="fig-topology">
  <img src="figures/fig-topology.svg" style="width:100%" alt="An 8-leaf EML with two algorithms showing shared topology with independent hash values per algorithm.">
  <figcaption><strong>Figure 1.</strong> An 8-leaf EML with two algorithms. Algorithm A (blue) is active from genesis. Algorithm B (amber) was added at position 4; its pre-activation positions are filled with null constants (gray, dashed). Each node computes independent hash values per algorithm over a shared topology.</figcaption>
</figure>
```
