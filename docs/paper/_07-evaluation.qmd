...
While the complexity regression tests confirm asymptotic scaling, we also evaluate absolute performance to demonstrate practical viability. We benchmark inclusion proof generation using the standard SHA-256 hash algorithm and an in-memory storage backend, running on a single thread of a standard server CPU (using thread-time measurements to isolate scheduler noise). 
