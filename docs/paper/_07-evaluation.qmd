...
**Comparative Overhead Analysis.** When evaluated under identical storage backend configurations, the database query footprint of the EML is identical to that of a vanilla, single-algorithm RFC 9162 Merkle tree. For active subtrees, EML's `subtree_root` queries resolve internal nodes by issuing the exact same coordinate lookups `get_node(alg_id, left, height)` to the storage layer as a standard RFC 9162 tree. Because `subtree_root` intercepts null intervals in memory via the local `NullTable`, the storage engine completely avoids database query duplication. The EML projection matches the physical database I/O footprint of a dedicated single-algorithm MALT exactly. EML does not merely approximate MALT performance; it inherits MALT's database footprint by routing structural sparsity through in-memory coordinate constraints.
