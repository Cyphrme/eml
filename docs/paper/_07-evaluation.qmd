...
exponentially increasing sizes ($n = 2^4$ through $2^{16}$), measures
...
\begin{figure*}[t]
  ...
  range from $2^4$ to $2^{20}$ (${\sim}$1M) leaves; each point is the
  median of 31~trials (5~warmup) via per-thread CPU time (IQR
  $<$\,5\% of median in all cases).
  (a)~Inclusion proof generation scales as $O(\log n)$ (dashed fit).
  (b)~Consistency proof generation is bounded by a constant
  (${\leq}1.2\,\mu$s) across five orders of magnitude.}
...
- **Consistency proofs**: Theoretically $\mathcal{O}(\log n)$, but
  empirically constant across four orders of magnitude
  (@fig:complexity(b)). The `subtree_root` engine resolves each
