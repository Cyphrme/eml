...
The implementation is validated by 74 tests across four categories:
...
| Complexity regression | 6 | Empirical curve fitting against formal bounds ($\mathcal{O}(\log n)$ proofs, $\mathcal{O}(1)$ amortized append, $\mathcal{O}(\log K)$ addition, $\mathcal{O}(\log n)$ gap resumption) |
...
complexity class using the `bigoish` library. Consistency proof
benchmarks sample `old_size` from five split points
$\{n/4, n/3, n/2, 2n/3, 3n/4\}$ per tree size to avoid biasing toward
the favorable power-of-two bisection case.

```{=latex}
\begin{figure*}[t]
  \centering
  \input{figures/fig-complexity.tex}
  \caption{Empirical proof generation time vs.\ tree size. Tree sizes
  range from $2^4$ to $2^{24}$ (${\sim}$16M) leaves; each point is the
  median of 31~trials (5~warmup) per configuration via per-thread CPU
  time. Consistency proofs sample five split points per tree size
  ($n/4$, $n/3$, $n/2$, $2n/3$, $3n/4$); the IQR reflects variation
  across proof topologies, not measurement noise.
  (a)~Inclusion proof generation scales as $O(\log n)$ (dashed fit).
  (b)~Consistency proof generation is bounded by a constant
  independent of tree size across six orders of magnitude.}
  \label{fig:complexity}
\end{figure*}
```

The fitted curves confirm:

- **Inclusion proofs**: $\mathcal{O}(\log n)$. Proof generation time
  scales logarithmically with tree size across all tested
  configurations (@fig:complexity(a)).
- **Consistency proofs**: Theoretically $\mathcal{O}(\log n)$, but
  empirically constant across six orders of magnitude
  (@fig:complexity(b)). Measured across diversified split points
  ($n/4$ through $3n/4$), the `subtree_root` engine resolves each
  sibling hash via a single stored-node lookup; the logarithmic number
  of lookups is dominated by per-call overhead at these tree sizes.
...
Four `cargo-fuzz` harnesses exercise adversarial inputs against the
proof verification and elision surfaces:

::: {#tbl-fuzz-targets}
| Target | Runs | Panics |
|:-------|-----:|-------:|
| `verify_inclusion` | ~500K | 0 |
| `verify_consistency` | ~500K | 0 |
| `rehydrate_proof` | ~500K | 0 |
| `proof_mutation` | ~500K | 0 |
| **Total** | **~2M** | **0** |

: Fuzz target coverage summary.
:::
...
that the mutated proof is rejected. Zero panics across approximately 2
million combined runs demonstrate that the proof verification surface
handles arbitrary malformed inputs gracefully.
...
operations, checking all invariants (\lawname{A-Equiv},
\lawname{A-Stack}, \lawname{I-Sound}, \lawname{K-Sound},
\lawname{T-Bound}, \lawname{Proj-Valid}) after each operation. The
