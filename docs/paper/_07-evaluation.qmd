...
: Test suite composition. {#tbl-test-categories}
:::
...
<!-- TODO: Figure 5 — Inclusion proof time vs tree size (log-log) -->
<!-- TODO: Figure 6 — Consistency proof time vs tree size (log-log) -->

The fitted curves confirm:

- **Inclusion proofs**: $\mathcal{O}(\log n)$. Proof generation time
  scales logarithmically with tree size across all tested configurations.
- **Consistency proofs**: $\mathcal{O}(\log n)$. The `subtree_root`
  engine resolves sibling hashes via stored-node lookups, maintaining
  logarithmic scaling even for trees with multiple active algorithms.
...
: Fuzz target coverage summary. {#tbl-fuzz-targets}
:::
