...
However, this shifts the bandwidth cost to a one-time session-level bootstrap payload: clients must download the activation map $\text{act}$. For $|A|$ algorithms with $E$ epochs, this requires distributing a metadata payload of size $\mathcal{O}(|A| \cdot E)$. Because algorithm lifecycle operations are rare (months or years), this cost is amortized over millions of subsequent proofs, resulting in a net wire overhead of zero per proof. This contrasts with SMT compression, which requires transmitting an explicit bitmap (typically 32 bytes) with *every* proof [@laurie2012; @tari2018].
