...
For restarts, the `from_storage` constructor reconstructs the full EML state in $\mathcal{O}(\log n)$ per algorithm. It loads the epoch metadata, decomposes the tree size $n$ into binary components, and rebuilds the frontier stacks by resolving the root of each complete subtree through the node store, bypassing the need to replay the append history.
