...
*   **Unified Verification:** The verifier always requires a valid `log_arity` ($k \geq 2$). Given the trusted `index`, `tree_size`, and `log_arity`, it computes the expected log-level path length from the frontier structure (`frontier_for_size`). The proof path is then dynamically split: the trailing `expected_len` steps are the **log skeleton** and are topologically verified via `reconstruct_index_from_path` (asserting that sibling counts and positions match the deterministic $k$-ary topology). Any leading steps beyond the expected log-level length are the **subtree portion** — these are verified purely by hash chaining (cryptographic membership) without topological assertions, since subtree arities are application-defined and non-uniform.
