...
impl FaultHandle {
    ...
    fn enable_drop(&self) {
        self.corruptions.set(0);
        self.mode.set(FaultMode::Drop);
    }

    /// Disable fault injection.
    #[allow(dead_code)]
    fn disable(&self) {
        self.mode.set(FaultMode::Clean);
    }
...
///
/// Fault mode is controlled via a shared `Rc<Cell<FaultMode>>` — the
...
impl CorruptingStorage {
    ...
    fn new() -> (Self, FaultHandle) {
        let mode = Rc::new(Cell::new(FaultMode::Clean));
        let corruptions = Rc::new(Cell::new(0u64));
...
impl Storage for CorruptingStorage {
    ...
    fn get_node(
        ...
        let result = match &mode {
            FaultMode::Clean => Ok(self.nodes.get(&(alg_id, left, height)).cloned()),
            FaultMode::BitFlip { byte_offset, bit } => {
                Ok(self.nodes.get(&(alg_id, left, height)).map(|hash| {
            ...
            FaultMode::Drop => {
                if self.nodes.contains_key(&(alg_id, left, height)) {
                    self.corruptions.set(self.corruptions.get() + 1);
                }
                Ok(None)
            },
...
fn build_log(leaf_count: u64) -> (Log<CorruptingStorage>, FaultHandle) {
