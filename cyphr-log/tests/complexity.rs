...
use cyphr_log::{Hasher, MemoryStorage, NaryMerkleLog, TreeConfig};
...
fn complexity_within_subtree_path_linear_in_nodes() {
    ...
    for &d in depths {
        ...
        for _ in 0..trials {
            let start = ThreadTime::now();
            for _ in 0..50 {
                let _ = cyphr_log::within_subtree_path(&Sha256Hasher, &subtree, 0);
            }
    ...
    for &n in sizes {
        ...
        for _ in 0..trials {
            ...
            for _ in 0..50 {
                ...
                let mut path =
                    cyphr_log::within_subtree_path(&Sha256Hasher, first_subtree, 0).unwrap();
